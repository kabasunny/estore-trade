...
このシステムは、立花証券APIを利用して株式の自動売買を行うためのものです。主な機能は以下の通りです。

*   **立花証券APIとの連携:**
    *   立花証券のシステムと通信し、株式の注文、注文状況の確認、注文キャンセルを行います。
*   **注文管理:**
    *   ユーザーの注文（銘柄、時刻、価格、数量）を記録・管理します。
*   **リアルタイムイベント処理:**
    *   立花証券システムからの情報（約定通知、システム状態変更など）をリアルタイムに受信し、適切な処理を行います。
*   **データベース連携:**
    *   注文情報などをデータベースに保存し、後で参照できるようにします。
*   **ログ記録:**
    *   プログラムの動作状況（エラー、注文成功など）を詳細に記録します。

## 処理フロー

1.  **初期設定:**
    *   設定ファイル（.env）からAPIキー、データベース接続情報などを読み込みます。
    *   ログ記録の準備を行います。
    *   データベースに接続します。
    *   立花証券APIクライアントを初期化します。

2.  **立花証券APIへのログインとマスタデータ取得:**
    *   立花証券APIにログインし、API利用の準備を整えます。
    *   銘柄情報、呼値などの取引に必要な情報をダウンロードします。

3.  **データベースの準備:**
    *   注文情報を管理する「注文リポジトリ」を準備します。
    *   口座情報を管理する「口座リポジトリ」を準備します。

4.  **取引機能の準備:**
    *   注文、注文状況確認、注文キャンセルなどの機能（取引ユースケース）を準備します。

5.  **リアルタイムイベント受信の準備:**
    *   立花証券システムからのイベントを受信するための「イベントストリーム」を準備します。
    *   イベントストリームを開始し、イベント受信待機状態にします。
    *   受信したイベントは取引ユースケースに送られ、適切な処理が行われます。

6.  **Webサーバー起動:**
    *   外部からの注文リクエストを受け付けるWebサーバーを起動します（`/trade` エンドポイントで注文を受け付け）。

7.  **注文受付と処理 (Webサーバー経由):**
    *   Webブラウザなどから `/trade` へ注文リクエストを受信します。
    *   リクエスト内容（銘柄、数量、価格、売買区分）を検証します。
        *   数量が正の数であるかなど、リクエストの妥当性をチェックします。
    *   取引ユースケースを用いて立花証券に注文を発注します。
    *   注文成功後、注文情報をデータベースに保存します。
    *   注文結果をWebブラウザに返します。

8.  **プログラム終了処理:**
    *   プログラム終了指示（Ctrl+Cなど）を受け取ると、以下の処理を行います。
        *   イベントストリームを停止します。
        *   Webサーバーを停止します。
        *   データベース接続を閉じます。
        *   ログを記録して終了します。
...
*   **リポジトリ:** データベースとのやり取りを専門に行うコンポーネントです。
*   **ユースケース:** 注文や口座管理など、特定の機能を実現する一連の処理をまとめたものです。
*   **イベントストリーム:** 立花証券システムからリアルタイムに情報を受け取るための仕組みです。
*   **ゴルーチン:** イベント受信やWebサーバー処理など、時間のかかる処理を他の処理と並行して実行し、プログラム全体の応答性を向上させています。
*   **Long Polling:** イベントストリームで利用されている、サーバーからクライアントへ情報をプッシュする技術です。
