...
# 2. 株式新規注文
# ----------------------
def new_order(session, request_url,  zyoutoeki_kazei_c, issue_code, baibai_kubun, order_price, order_suryou, second_password):
    """株式新規注文を行う."""
...
# 3. 株式訂正注文
# ----------------------
def correct_order(session, request_url, order_number, eigyou_day, order_price, second_password):
    """株式訂正注文を行う."""
...
if __name__ == "__main__":
    ...
    # 2. 新規注文
    new_order_result = new_order(login_info, request_url, zyoutoeki_kazei_c, issue_code, baibai_kubun, order_price, order_suryou, second_password)
    ...
    if new_order_result:
        ...
        # 3. 訂正注文 (指値に変更)
        correct_order_result = correct_order(login_info, request_url, order_number, eigyou_day, "12000", second_password)

        if correct_order_result:
            # 4. 取消注文
            cancel_order_result = cancel_order(login_info, request_url, order_number, eigyou_day, second_password