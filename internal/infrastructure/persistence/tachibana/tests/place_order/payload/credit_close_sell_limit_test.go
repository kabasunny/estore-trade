// internal/infrastructure/persistence/tachibana/tests/payload/place_order_credit_close_sell_limit_test.go
...
// TestPlaceOrder_CreditCloseSellLimitPayload は信用返済売り指値注文のペイロードをテストします。
func TestPlaceOrder_CreditCloseSellLimitPayload(t *testing.T) {
	t.Run("正常系: 信用返済売り指値注文のペイロードが正しく生成されること", func(t *testing.T) {
		// テスト用の TachibanaClientImple インスタンスを作成
		tc := tachibana.CreateTestClient(t, nil)

		// domain.Order オブジェクトを作成 (信用返済売り指値注文)
		order := &domain.Order{
			Symbol:     "7974",               // 例: 任天堂
			Side:       "short",              // 売り
...
// go test -v ./internal/infrastructure/persistence/tachibana/tests/payload/place_order_credit_close_sell_limit_test.go
