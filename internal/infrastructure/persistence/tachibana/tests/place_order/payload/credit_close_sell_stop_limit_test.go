// internal/infrastructure/persistence/tachibana/tests/payload/place_order_credit_close_sell_stop_limit_test.go
...
// TestPlaceOrder_CreditCloseSellStopLimitPayload は信用返済売り通常+逆指値注文のペイロードをテストします。
func TestPlaceOrder_CreditCloseSellStopLimitPayload(t *testing.T) {
	t.Run("正常系: 信用返済売り通常+逆指値注文(トリガー後指値)のペイロードが正しく生成されること", func(t *testing.T) {
		// テスト用の TachibanaClientImple インスタンスを作成
		tc := tachibana.CreateTestClient(t, nil)

		// domain.Order オブジェクトを作成 (信用返済売り通常+逆指値注文)
		order := &domain.Order{
			Symbol:                "7974",                    // 例: 任天堂
			Side:                  "short",                   // 売り
			OrderType:             "credit_close_stop_limit", // 信用返済 (通常+逆指値)
			Quantity:              100,
			Price:                 8000.0,  // 通常注文の価格（指値）
			TriggerPrice:          9500.0,  // 逆指値トリガー価格
			MarketCode:            "00",    // 東証
			AfterTriggerOrderType: "limit", // トリガー後指値
			AfterTriggerPrice:     9000.0,  // トリガー後指値
	...
	t.Run("正常系: 信用返済売り通常+逆指値注文(トリガー後成行)のペイロードが正しく生成されること", func(t *testing.T) {
		// テスト用の TachibanaClientImple インスタンスを作成
		tc := tachibana.CreateTestClient(t, nil)

		// domain.Order オブジェクトを作成 (信用返済売り通常+逆指値注文 - トリガー後成行)
		order := &domain.Order{
			Symbol:                "7974",                    // 例: 任天堂
			Side:                  "short",                   // 売り
			OrderType:             "credit_close_stop_limit", // 信用返済 (通常+逆指値)
...
// go test -v ./internal/infrastructure/persistence/tachibana/tests/payload/place_order_credit_close_sell_stop_limit_test.go
