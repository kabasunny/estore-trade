// internal/infrastructure/persistence/tachibana/tests/payload/place_order_credit_close_buy_stop_test.go
...
// TestPlaceOrder_CreditCloseBuyStopPayload は信用返済買い通常逆指値注文のペイロードをテストします。
func TestPlaceOrder_CreditCloseBuyStopPayload(t *testing.T) {
	t.Run("正常系: 信用返済買い通常逆指値注文(トリガー後指値)のペイロードが正しく生成されること", func(t *testing.T) {
		// テスト用の TachibanaClientImple インスタンスを作成
		tc := tachibana.CreateTestClient(t, nil)

		// domain.Order オブジェクトを作成 (信用返済買い通常逆指値注文)
		order := &domain.Order{
			Symbol:       "7974",              // 例: 任天堂
			Side:         "long",              // 買い
	...
	t.Run("正常系: 信用返済買い通常逆指値注文(トリガー後成行)のペイロードが正しく生成されること", func(t *testing.T) {
		// テスト用の TachibanaClientImple インスタンスを作成
		tc := tachibana.CreateTestClient(t, nil)

		// domain.Order オブジェクトを作成 (信用返済買い通常逆指値注文 - トリガー後成行)
		order := &domain.Order{
			Symbol:       "7974",              // 例: 任天堂
			Side:         "long",              // 買い
			OrderType:    "credit_close_stop", // 信用返済 (通常逆指値)
...
// go test -v ./internal/infrastructure/persistence/tachibana/tests/payload/place_order_credit_close_buy_stop_test.go
